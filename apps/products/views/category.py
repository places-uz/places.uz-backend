...
class CategoriesListView(APIView):
    ...
    def get(self, request, place_url, format=None):
        queryset = Category.objects.filter(
            place__url=place_url, place__owner=request.user)
...
class CategoriesDetailView(APIView):
    ...
    def get_object(self, category_id, request):
        try:
            return Category.objects.get(id=category_id, place__owner=request.user)
        except Category.DoesNotExist:
            raise Http404

    def get(self, request, place_url, category_id, format=None):
        category = self.get_object(category_id, request)
